...
import { createReview, getVetById, markCompleteReview } from "../services/vetService";
...
export default function GiveFeedbackPage() {
  ...
  const { vetId, appointment_id } = useLocalSearchParams<{
    vetId: string;
    appointment_id: string;
  }>();
  const [vet, setVet] = useState<Vets>();
  ...
  const isSubmitDisabled =
    rating === 0 || currentWordCount === 0 || currentWordCount > WORD_LIMIT;
  ...
  const handleSubmit = async() => {
    ...
    try {
      ...
      await markCompleteReview(appointment_id);
    ...
    <View style={styles.container}>
      ...
      >
          ...
          >
            ...
            <Text style={styles.doctorName}>{vet?.full_name} </Text>
            <Text style={styles.doctorTitle}>{vet?.doctorProfile?.degree}</Text>

            <Text style={styles.doctorTitle}>{vet?.doctorProfile?.tags}</Text>
            ...
            <View style={styles.buttonGroup}>
              ...
              <CommonButton
                ...
                backgroundColor={
                  isSubmitDisabled ? Colors.gray : Colors.primary
                }
